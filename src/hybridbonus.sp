...
{
	name = "Hybrid Bonus",
	author = "CanadaRox, Stabby, scient",
	description = "Custom hybrid scoring based on damage, distance, and permanent health.",
	version = "1",
...
public OnPluginStart()
{
	...
	hPermMulti = CreateConVar("sm_permfactor", "0.5", "Permanent health is divided by this factor before giving points to survivors.", FCVAR_PLUGIN, true, 0.0, true, 2.0);
...
stock GetSurvivorPermanentHealth_(client) return GetEntProp(client, Prop_Send, "m_currentReviveCount") > 0 ? 0 : (GetEntProp(client, Prop_Send, "m_iHealth") > 0 ? GetEntProp(client, Prop_Send, "m_iHealth") : 0);

stock GetTotalPermanentHealth()
{
	new healthSum;
	new iSurvivorCount;
	...
	for (new i = 1; i <= MaxClients && iSurvivorCount < maxSurvs; i++)
	{
		...
		{
			iSurvivorCount++;
			if (IsPlayerAlive(i)) healthSum+=GetSurvivorPermanentHealth_(i);
		}
	}
	return healthSum;
}
...
stock CalculateSurvivalBonus()
{
	...
	{
		PrintToChatAll("Perma health modified sum (total_health / modifier) (f): %f", MAX(GetTotalPermanentHealth() * GetConVarFloat(hPermMulti), 0.0));
		PrintToChatAll("FINAL BONUS: %d", RoundToFloor(MAX(GetConVarFloat(hMaxDamageCvar) - GetDamage() * GetConVarFloat(hDamageMultiCvar), 0.0) / 4 + MAX(GetTotalPermanentHealth() * GetConVarFloat(hPermMulti), 0.0) + GetConVarFloat(hStaticBonusCvar)));
		return RoundToFloor(MAX(GetConVarFloat(hMaxDamageCvar) - GetDamage() * GetConVarFloat(hDamageMultiCvar) + GetTotalPermanentHealth() * GetConVarFloat(hPermMulti), 0.0) / 4 + GetConVarFloat(hStaticBonusCvar));
	}
	else
	{
